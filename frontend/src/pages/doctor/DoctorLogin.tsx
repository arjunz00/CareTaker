...
export default function DoctorLogin() {
  ...
  const { login, loading, error } = useDoctor();
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl relative space-y-6">
        ...
        <div className="text-center">
          ...
          <h2 className="text-lg font-bold text-white uppercase tracking-wider">Clinician Portal Gateway</h2>
          <p className="text-xs text-slate-400 mt-1">Authenticate to access assigned patient telemetry indices.</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4 text-xs select-none">
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Clinician Email Address</label>
            <input 
              type="email" 
              required 
              value={email}
          ...
          >
            {loading ? "Authenticating Clinician..." : "Sign In"}
          </button>
        </form>
        ...
        <div className="border-t border-slate-850 pt-4 flex gap-2.5 items-start text-[10px] text-slate-500 select-none">
          ...
          <p className="leading-relaxed">
            <strong>Secure healthcare access:</strong> Session logins are recorded in audit logs. Patient records access requires clinical credentials scope matching HIPAA guidelines.
          </p>
        </div>
